...
import { predictSoil } from '@/services/mlApi';
...
export default function SoilPredictionPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      toast.error('Analysis failed', err.message || 'Failed to analyze soil');
    } finally {
    ...
    <div className="min-h-screen bg-[#f7fbf6] flex">
      ...
      <aside className="w-[260px] bg-[#e9f5ee] border-r border-black/5 flex flex-col flex-shrink-0">
        ...
        <nav className="flex-1 px-4 py-6 flex flex-col gap-1">
          ...
          <div className="mb-6">
            ...
            <NavItem href="/dashboard/soil-prediction" icon={Layers} label="Soil Prediction" active={pathname === '/dashboard/soil-prediction'} />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 bg-white border-b border-black/5 flex items-center justify-between px-8 flex-shrink-0 sticky top-0 z-10">
          <div className="text-[15px] font-medium text-[#0f1720]">Soil Prediction</div>
        ...
        <main className="flex-1 p-10 overflow-y-auto">
          <div className="max-w-5xl mx-auto">
            ...
            <motion.div
              ...
              className="mb-8"
            >
              <h1 className="text-2xl font-semibold text-[#0f1720] mb-2">Soil Analysis</h1>
              <p className="text-sm text-[#6b7276] max-w-xl leading-relaxed">
                Upload a soil image to determine soil type and receive optimized crop recommendations for your land.
              </p>
            </motion.div>
                ...
                {result && (
                  <div className="mt-6 p-5 bg-[#e6f6ea] border border-[#2f9a4a]/20 rounded-lg">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        <h4 className="text-sm font-medium text-[#6b7276]">Detected Soil Type</h4>
                        <p className="text-xl font-bold text-[#2f9a4a]">{result.soilType}</p>
                      </div>
                    </div>
                    <p className="text-sm text-[#6b7276]">Template: {result.templateName}</p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white border border-black/5 rounded-lg p-6">
                ...
                <ul className="flex flex-col gap-6">
                  <StepItem 
                    ...
                    description="Capture a clear image of your soil surface."
                  />
                  <StepItem 
                    icon={Upload} 
                    title="2. Upload image" 
                    description="Submit the photo for AI analysis."
                  />
                  <StepItem 
                    icon={Microscope} 
                    title="3. AI Analysis" 
                    description="Our model identifies your soil type."
                  />
                  <StepItem 
                    icon={Mountain} 
                    title="4. Get Recommendations" 
                    description="Receive crop suggestions for your soil."
                  />
                </ul>
              </div>
            </motion.div>
            ...
            >
              <div className="px-6 py-5 border-b border-black/5">
                <h2 className="text-base font-semibold text-[#0f1720]">Recent Soil Analyses</h2>
              </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-[#f0f3f2]">
                      ...
                      <th className="text-left px-6 py-3 text-xs font-medium text-[#6b7276]">Soil Type</th>
                  ...
                  <tbody>
                    <tr className="border-b border-black/5 last:border-b-0 hover:bg-[#f7fbf6] transition-colors cursor-pointer">
                      ...
                      <td className="px-6 py-4 text-sm text-[#0f1720]">Alluvial Soil</td>
                      <td className="px-6 py-4 text-sm text-[#0f1720]">Oct 12, 2023</td>
                      <td className="px-6 py-4">
                        <span className="px-2.5 py-1 rounded-full text-xs font-medium bg-[#2f9a4a] text-white">Completed</span>
                      </td>
                    </tr>
                  </tbody>
                </table>
