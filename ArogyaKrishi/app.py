...
# Lazy-load pipelines to avoid startup errors
# These will be loaded on first use instead of at module import
_leaf_disease_pipe_cache = None

def get_leaf_disease_pipeline():
    """Load the object-detection pipeline on first use (lazy loading)"""
    global _leaf_disease_pipe_cache
    if _leaf_disease_pipe_cache is None:
        print("Loading leaf disease detection model...")
        try:
            _leaf_disease_pipe_cache = pipeline("object-detection", model="krifa/LeafDiseaseDetection")
        except Exception as e:
            print(f"Warning: Could not load LeafDiseaseDetection model - {e}")
            print("Using fallback...")
            _leaf_disease_pipe_cache = None
    return _leaf_disease_pipe_cache
...
def get_disease_model():
    ...
    if _disease_model_cache is None:
        print("Loading local plant disease model...")
        _disease_model_cache = ResNet9(3, len(disease_classes))
        _disease_model_cache.load_state_dict(torch.load(
...
soil_type_prediction_model_path = 'model/DenseNet121v2_95.h5'

labels = ['Chalky Soil', 'Mary Soil', 'Sand', 'Slit Soil', 'Alluvial Soil', 'Black Soil', 'Clay Soil', 'Red Soil']

# Custom layer class to handle DepthwiseConv2D with groups parameter
...
# Custom objects to handle incompatible layer configurations
custom_objects = {
...
def load_model_safely(model_path):
    ...
    try:
        ...
        try:
            ...
            with tf.keras.utils.custom_object_scope(custom_objects):
                model = tf.keras.models.load_model(model_path, compile=False)
            print(f"Model loaded with custom objects: {model_path}")
            return model
...
def create_fallback_model():
    """Create a simple fallback model when original model fails to load"""
...
pipe = None  # Placeholder for lazy loading

# Lazy-load the image classification pipeline (to avoid startup errors with PyTorch versions)
_pipe_cache = None

def get_pipeline():
    """Load the image classification pipeline on first use (lazy loading)"""
    global _pipe_cache
    if _pipe_cache is None:
        print("Loading local plant disease model...")
        try:
            # Use local model
            def local_predict(img):
                ...
                with torch.no_grad():
                    outputs = get_disease_model()(img_t)
                    probabilities = torch.nn.functional.softmax(outputs[0], dim=0)
                    top5_prob, top5_catid = torch.topk(probabilities, 5)
                    results = []
                    for i in range(top5_prob.size(0)):
                        results.append({
                            'label': disease_classes[top5_catid[i]],
                            'score': top5_prob[i].item()
                        })
                    return results
            _pipe_cache = local_predict
        except Exception as e:
            print(f"Warning: Could not load local model - {e}")
            print("Using fallback model instead")
            _pipe_cache = pipeline("image-classification", model="google/vit-base-patch16-224")
    return _pipe_cache
...
# ---------- New JSON API endpoints for Next.js ----------
    ...
    try:
        ...
        if prediction:
            ...
            return jsonify({
                ...
                'fertilizerRecommendation': fertilizer_info
            })
        else:
            return jsonify({'error': 'No predictions made'}), 500
    except Exception as e:
        print(f"API disease detection error: {e}")
        return jsonify({'error': 'An error occurred during prediction'}), 500


@app.route('/api/soil-prediction', methods=['POST'])
def api_soil_prediction():
    if 'soil_image' not in request.files:
        return jsonify({'error': 'No file part in the request'}), 400
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'soilType': predicted_soil,
            'templateName': template_name
        })
    except Exception as e:
        print(f"API soil prediction error: {e}")
...
def api_fertilizer_recommendation():
    try:
        ...
        # Use relative path for fertilizer data
        fertilizer_data_path = 'data/fertilizer.csv'
        
        # Check if file exists
        if not os.path.exists(fertilizer_data_path):
            return jsonify({'error': 'Fertilizer data file not found'}), 500
        ...
        if crop_data.empty:
            # Return default recommendation if crop not found
            recommendation_html = str(fertilizer_dic.get('Nlow', 'Use balanced NPK fertilizer as per soil test.'))
            key = 'Nlow'
        else:
            ...
            recommendation_html = str(fertilizer_dic.get(key, 'Use balanced NPK fertilizer as per soil test.'))
        
        return jsonify({
            'success': True,
            'recommendationHtml': recommendation_html,
            'key': key,
            'inputs': {'cropName': crop_name, 'N': N, 'P': P, 'K': K}
        })
    except KeyError as e:
        print(f"API fertilizer recommendation error - Missing field: {e}")
        return jsonify({'error': f'Missing required field: {str(e)}'}), 400
    except ValueError as e:
        print(f"API fertilizer recommendation error - Invalid value: {e}")
        return jsonify({'error': f'Invalid input value: {str(e)}'}), 400
    except Exception as e:
        print(f"API fertilizer recommendation error: {e}")
        return jsonify({'error': f'An error occurred: {str(e)}'}), 500
